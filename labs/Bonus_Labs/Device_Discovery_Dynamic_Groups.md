...
As soon as the playbook reaches the `group_by` task, it'll create dynamic groups using the `key` paramemter resulting in new groups that map back to the `ansible_device_os` variable and the `debug` will expose a built in variable called `groups` which will show a dictionary of keys that are all group names defined in the inventory file and values are list of host names that are members of the group.
...
ok: [vmx2] => {
    "inventory_hostname": "vmx2"
}
ok: [vmx3] => {
    "inventory_hostname": "vmx3"
}

PLAY RECAP **********************************************************
csr1                       : ok=7    changed=0    unreachable=0    failed=0   
csr2                       : ok=7    changed=0    unreachable=0    failed=0   
csr3                       : ok=7    changed=0    unreachable=0    failed=0   
nxos-spine1                : ok=7    changed=0    unreachable=0    failed=0   
nxos-spine2                : ok=7    changed=0    unreachable=0    failed=0   
vmx1                       : ok=7    changed=0    unreachable=0    failed=0   
vmx2                       : ok=7    changed=0    unreachable=0    failed=0   
vmx3                       : ok=7    changed=0    unreachable=0    failed=0   
